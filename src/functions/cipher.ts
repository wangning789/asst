...
/** url header 乱码解码
 * @param {string} param header中返回的url
 *
 * @returns {string} key 解码后的正常文字
 */

function decodeUrl(param: string) {
  const res = escape(param)
  return decodeURI(res)
}

export { pwd, encode, decode, encrypt, decrypt, decodeUrl }
