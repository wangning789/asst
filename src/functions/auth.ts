...
import { Menu as OrginMenu } from '@/api/authServer/menu'
...
AUTH.AUTH || (AUTH.AUTH = {})
/** 重建菜单树 */
function restructure(menu: OrginMenu): OrginMenu {
  // 深度优先
  ...
  let cLen
  while (index--) {
    child = (children as OrginMenu[])[index]
    if ((cLen = child.children?.length)) {
      if (child.rootNode) {
        if (child.menuCode) {
          ;(children as OrginMenu[]).splice(index - 1, 0, ...child.children)
          index += cLen
          delete child.children
        } else {
          ;(children as OrginMenu[]).splice(index, 1, ...child.children)
          index += cLen - 1
        }
        child = (children as OrginMenu[])[index]
      }
...
/** 重建权限信息 */
function rebuild(menu: OrginMenu) {
