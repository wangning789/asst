...
import { get, post, CancelToken } from './ajax'
 ...
 */
function download(url: string, query?: IObject, name?: string, method?: string) {
  const source = CancelToken.source()
  let fn
  if (method === 'post') {
    fn = post(url, query, null, {
      responseType: 'blob',
  ...
  const promise = fn
    .then(res => {
      ...
      if (!name) {
        name = res.headers['content-disposition'] ? res.headers['content-disposition'].split(';') : ''
        if (name) {
          name = (name as any)[(name as any).length - 1].split('=')
          name = (name as any)[(name as any).length - 1]
        }
      }
      if (window.navigator.msSaveBlob) { // 兼容处理ie浏览器不能下载的问题
        // 要在取res的地方判断是否有值，ie浏览器在downloader的doemload方法不返回值
