    ...
    >
      ...
      >
        ...
        <ElRow>
          ...
          <ElCol :span="12">
            <label>发布服务器IP：</label>
            <span>{{ item.publicServerIp }}</span>
          </ElCol>
        </ElRow>

        <h4>Web服务器信息</h4>
        <ElTable
          :data="item.tamperServerDTOList"
...
import { tamperInfo, baseInfo } from '@/api/servicePlatform/safetyAsset'
...
export default {
  ...
  created() {
    tamperInfo(this.$route.params.id).then(res => {
