...
import { listByBusinessId, getIncidentCount } from '@/api/systemManage/organization'
...
export default {
  ...
  methods: {
    ...
    async listByBusinessId() {
      const res = await listByBusinessId.apply(null, arguments).finally(() => {
  ...
  mounted() {
    this.filterForm.hostId = this.hostId
    getIncidentCount({ hostId: this.hostId }).then(res => {
