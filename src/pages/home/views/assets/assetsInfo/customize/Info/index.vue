      ...
      >
        <div :class="$style.rr">
          ...
          <ElButton
            plain
            size="mini"
            @click="$router.push(`/assetsInfo/customize/${$route.params.ciTypeId}/
...
import {
  ...
  getBaseCi,
  changeStatus,
} from '@/api/assets/assetsInfo/customize'
// import { fixColumnDTOList } from '@/api/assets/assetsInfo/index'
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ciTypeDetail(this.$route.params.ciTypeId).then(res => {
      ...
      const data = (await getBaseCi(param)).data.body
      // 固定字段
      this.columnDTOList = data.columnDTOList.filter(child => child.display === '1' &&
       child.allowModify === '2' &&
        ...
        child.enName !== 'ciSysCode' && child.enName !== 'useStatus')
      // 自定义字段
