<!-- 描述 -->
<template>
  <div>
    <!--自定义资产 -->
    <div>
      <IFilter
        v-model="filterForm"
        :ci-type-id="ciTypeId"
        :checked-org-list="checkedOrgList"
        :active-name="ciName"
        @Refresh="Refresh"
      />
      <div :class="$style.severlist">
        <div :class="$style.table">
          <ITable
            ref="table"
            :get="get"
            :query="filterForm"
            :head="head"
          >
            <template #prefix>
              <ElTableColumn
                width="180"
                fixed
                prop="ciSysCode"
                label="系统编号"
              >
                <template #default="{ row }">
                  <RouterLink
                    class="el-button el-button--text"
                    :to="`/assetsInfo/customize/${ciTypeId}/${row.id}/info`"
                  >
                    {{ row.ciSysCode }}
                  </RouterLink>
                </template>
              </ElTableColumn>
            </template>
          </ITable>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
// @ts-nocheck
import IFilter from './Filter'
import { baseCiListAll, listCurrAndNext } from '@/api/assets/assetsInfo/customize'
import { ROLES } from '@/enums'
/// 常量(UPPER_CASE), 单例/变量(camelCase), 函数(无副作用,camelCase)
import STYLE from '@/scss/status.module.scss'
import ITable, { ElTableColumn } from '@com/Table'
export default {
  props: {
    checkedOrgList: { type: Array, default: () => { return [] } },
    activeName: { type: String, default: () => { return [] } },
    ciTypeId: { type: String, default: () => { return '' } },
    ciName: { type: String, default: () => { return '' } },
  },
  components: {
    IFilter,
    ITable,
    ElTableColumn,
  },
  data() {
    return {
      filterForm: {},
      head: [],
      serveType: [], // 服务器类型
      dataList: [],
      columnDTOList: [],
    }
  },
  computed: {
    STYLE: () => STYLE,
  },
  watch: {
    ciTypeId(a, b) {
      if (a !== b) {
        this.$refs.table.fetch()
      }
    },
  },
  methods: {
    Refresh() {
      this.$refs.table.fetch()
    },
    async get() {
      let fn = ''
      switch (true) {
        case this.authFit(ROLES.systemAdmin):
          fn = baseCiListAll
          break
        case this.authFit(ROLES.assetManager):
          fn = listCurrAndNext
          break
        default:
          break
      }
      const param = {
        ciTypeId: this.ciTypeId,
        ...this.filterForm,
        ...arguments[0],
      }
      const res = await fn(param)
      this.dataList = res.data.body.dataList.list.map(
        (element: any) => {
          return Object.assign(element, element.data)
        }
      )
      this.columnDTOList = res.data.body.columnDTOList
      const total = res.data.body.dataList.total
      this.columnDTOList.forEach(o => {
        // 是下拉的字段取XXName
        o.enName = o.type === '7' ? o.enName + 'Name' : o.enName
      })
      this.head = []
      // display  是否显示,1:是;2:否',
      // allowModify 是否固定字段,1:自定义字段;2:固定字段'
      this.columnDTOList.forEach(o => {
        if (o.enName === 'useStatusName') {
          this.head.push({
            prop: o.enName,
            label: o.chName,
            formatter: (row, col, cell) =>
              this.$createElement(
                'span',
                {
                  class: row.useStatus === '0' ? 'enable' : 'disable',
                },
                cell
              ),
          })
        } else if (o.display === '1' && o.enName !== 'ciSysCode') {
          this.head.push({ label: o.chName, prop: o.enName })
        }
      })
      res.data.body = {
        total: total,
        data: this.dataList,
      }
      return res
    },
  },
  activated() {
    // 重新更新列表
    this.filterForm = {
      ...this.filterForm,
      t: new Date().getTime(),
    }
  },
}
</script>

<style lang="scss" module>
.wrapper {
  overflow: hidden;
  color: #333;
  background-color: #fff;
  border: 1px solid #ebeef5;
  border-radius: 4px;
}

.table :global(.el-table__empty-block) {
  width: 100% !important;
}

.table :global(.el-table .cell) {
  white-space: nowrap;
}

</style>
