...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ...
      const fn = this.id ? serverInfo : serverGetColumn// 编辑或者添加显示字段接口
      const data = (await fn(this.id || '')).data.body
      this.columnDTOList = data.columnDTOList.filter(child => child.display === '1' && child.allowModify === '2' && child.enName !== 'belongCompanyId' && child.enName !== 'description' && child.enName !== 'add_time' && child.enName !== 'ciSysCode' && child.enName !== 'useStatus' && child.enName !== 'addTime' && child.enName !== 'country' && child.enName !== 'intranetIp' && child.enName !== 'interiorIp')
      // 自定义字段
    ...
    save() {
      ...
      const fn = this.id ? modifyServer : addServer
      fn(params).then(res => {
        if (res.data.status) {
